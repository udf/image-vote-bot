...
def register(callback_id):
  ...
  def wrapper(func):
    ...
    func.get_button = lambda text, extra_data=None: get_button(text, func, extra_data)
...
def get_button(text, func, extra_data=None):
...
async def dispatch(event):
  ...
  if not callback:
    ...
    return
  event.extra_data = cbor2.loads(event.data[1:])
